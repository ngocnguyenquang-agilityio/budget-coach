/**
 * AG-UI tool declaration for the client-side "calculate" tool.
 * Passed into `runAgent({ tools: [calculatorTool] }, ...)` so the model knows
 * it can offload arithmetic (e.g. "how many sunny days in a 30-day month
 * with 5 rainy days?") to a deterministic evaluator instead of guessing.
 */
export const calculatorTool = {
  ...
  parameters: {
    ...
    properties: {
      expression: {
        type: "string",
        description: 'The arithmetic expression to evaluate, e.g. "(30 - 5) * 2"',
      },
    },
    required: ["expression"],
  },
 ...
 * as a tool result, mirroring the non-throwing contract of `launchUrl`.
 */
