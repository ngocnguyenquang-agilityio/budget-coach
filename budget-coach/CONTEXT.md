...
- **Editing or deleting a Transaction corrects the record; it is not a money movement.** Deleting means "this never happened" — a recording mistake — never "I got my money back" (that is a Refund). Editing fixes what was misrecorded (merchant, note, amount, date, Category); it never changes a Transaction's Type, its Status (moving expected to received is Confirming, a different act), or the Savings Pot that funded it. Every edit or delete is confirmed by the User before it takes effect. Pot transfers, Schedule-generated expected Transactions, and Pot-funded Expenses cannot be edited or deleted this way — each is changed through the thing that owns it.
