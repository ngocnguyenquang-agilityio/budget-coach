...
- **Working memory scope**: only the Coach agent should carry `Memory`; its working memory schema must stay `{ savingsGoal, categoryLimits, lastReviewDate, pendingApproval }` with `scope: "resource"`. Transactions belong in LibSQL tables, not in working memory or shared state.
