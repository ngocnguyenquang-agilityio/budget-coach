  ...
  it("scores 1 when every proposed limit is within 50% of spend", async () => {
    const result = await adjustmentReasonablenessScorer.run({
      ...
      output: {
        ...
        threadId: "t1",
        analysis: {
          categoryTotals: [{ category: "Dining", committed: 0, spent: 100, overLimit: false, onTrackToExceed: false }],
          expenseTotal: 100,
  ...
  it("scores 0 for a category with net-negative spend when the proposed limit is way off", async () => {
    const result = await adjustmentReasonablenessScorer.run({
      ...
      output: {
        ...
        threadId: "t1",
        analysis: {
          categoryTotals: [{ category: "Dining", committed: 0, spent: -10, overLimit: false, onTrackToExceed: false }],
          expenseTotal: -10,
          committedExpenseTotal: 0,
          receivedIncome: 0,
          forecastIncome: 0,
  ...
  it("scores 0 when spend is zero but a nonzero limit is proposed", async () => {
    const result = await adjustmentReasonablenessScorer.run({
      ...
      output: {
        ...
        threadId: "t1",
        analysis: {
          categoryTotals: [{ category: "Dining", committed: 0, spent: 0, overLimit: false, onTrackToExceed: false }],
          expenseTotal: 0,
          committedExpenseTotal: 0,
          receivedIncome: 0,
          forecastIncome: 0,
