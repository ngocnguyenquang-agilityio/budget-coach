...
**What to build:** scorers attached to the relevant agents, giving observable quality signal in Mastra Studio. `categorizerAccuracyScorer` and `adjustmentReasonablenessScorer` are deterministic (no LLM judge); `coachScopeScorer` uses an LLM judge (see ADR-0005 — keyword matching couldn't catch paraphrased scope drift).
...
- [x] `coachScopeScorer` uses an LLM judge to grade Coach responses on a `none/mild/moderate/severe` scope-drift rubric, mapped to a `1/0.66/0.33/0` score (pairs with `financialAdviceGuardrail`)
...
- [x] `coachScopeScorer` is built via a `createCoachScopeScorer(judgeModel?)` factory so tests can inject a mock judge model instead of calling Cerebras
