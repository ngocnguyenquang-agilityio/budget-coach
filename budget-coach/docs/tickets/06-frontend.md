...
- [x] Frontend actions: `openAddTransactionForm` (pre-filled from chat) and `selectCategory` (filters the Transactions list to one category, exactly as if the user clicked that category's row)
...
- [x] `useAgentContext` reports the currently-visible month and any category the Transactions list is filtered to
