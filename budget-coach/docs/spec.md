...
- Tools (positional `execute(inputData, context)`): `listTransactionsTool`, `addTransactionsTool` (records one or more transactions in a single batch, then runs the income-drift check once against post-batch totals), `categorizeBatchTool` (agent-as-tool wrapper delegating to Categorizer; classifies one or more merchant + amount items in a single call, results in input order), `analyzeSpendingTool` (agent-as-tool wrapper delegating to Analyst), `setSavingsGoalTool` (writes into working memory), `approveBudgetTool` (suspends into the Monthly Review workflow).
