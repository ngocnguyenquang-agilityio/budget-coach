...
## Addendum: edits and deletes (ADR-0016)

The same pre-write baseline applies to editing or deleting a Transaction dated in a closed Period: before an applied row is written, record `netSavingsAtClose` for its Period unless one is already pending. An edit that moves a row's date between Periods touches both, so each closed one gets a baseline. Only Periods touched by a row that is actually applied get one — a row that fails validation leaves no amendment behind.
