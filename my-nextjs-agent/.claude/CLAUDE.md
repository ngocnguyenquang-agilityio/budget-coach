...
- `src/mastra/agents/weather-agent.ts` (id `weather-agent`) — reports current weather via `weatherTool`, and tracks a Fahrenheit/Celsius preference in working memory (`Memory()` with `workingMemory` enabled), settable via `setTemperatureUnitTool`. `Memory()` sets `options.lastMessages: 10` to bound how much thread history is sent to the model each turn; the chat client (`chat/page.tsx`) sends only the newest message per request via `DefaultChatTransport`'s `prepareSendMessagesRequest` rather than resending the full transcript, so `lastMessages` is the sole source of history bounding — per Mastra's own guidance, sending full client-side history is redundant with (and can conflict with) memory recall. Scored by `temperatureUnitScorer`.
